...
static void move_to_back(std::vector<std::shared_ptr<Process>> &processes,
                         std::vector<std::shared_ptr<Process>>::iterator &i);
static void set_current_state(Process &process, int time_quantum, Semaphore &s);

void Scheduler::schedule_processes(
    std::vector<std::shared_ptr<Process>> &processes, Semaphore &s) {
...
Scheduler::check_IO(std::vector<std::shared_ptr<Process>> &processes,
                    Semaphore &s) {
  ...
  for (auto i = processes.begin(); i != processes.end(); ++i) {
    ...
    if (process.get_current_instruction().type == "I/O") {
      ...
      if (process.get_current_instruction().end_critical &&
          process.is_in_critical()) {
...
void Scheduler::round_robin(std::vector<std::shared_ptr<Process>> &processes,
                            Semaphore &s) {
  ...
  while (time_quantum > 0) {
    if (temp <= process.process_map_vector.size()) {
      if (process.process_map(temp).runtime < time_quantum &&
          time_quantum > 0) {
        int total_runtime =
            process.get_total_runtime() - process.process_map(temp).runtime;
